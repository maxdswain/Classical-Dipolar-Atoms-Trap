...
void read_config(int *N, int *iterations, int *T, double *m, double *length, double *sigma, double *dipole_moment, double *dipole_unit_vector, double *frequency_z, double *frequency_transverse);
...
void particle_metropolis_hastings(double **positions, int iterations, int N, double m, int T, double sigma, double dipole_moment, double *dipole_unit_vector, double frequency_z, double frequency_transverse);
void export_positions(Double3D *positions_saved);

int main(int argc, char **argv) {
    int N, iterations, T;
    double m, length, sigma, dipole_moment, dipole_unit_vector[3], frequency_z, frequency_transverse;
    ...
    particle_metropolis_hastings(positions, iterations, N, m, T, sigma, dipole_moment, dipole_unit_vector, frequency_z, frequency_transverse);

    printf("Total energy: %e\n", calculate_total_energy(positions, N, m, dipole_moment, dipole_unit_vector, frequency_z, frequency_transverse));
...
void read_config(int *N, int *iterations, int *T, double *m, double *length, double *sigma, double *dipole_moment, double *dipole_unit_vector, double *frequency_z, double *frequency_transverse) {
    ...
    *dipole_moment = magnetic_dipole_moment.u.d;
    toml_array_t *magnetic_dipole_unit_vector_array = toml_array_in(properties, "dipole_unit_vector");
    for (int i = 0; i < 3; i++) {
        toml_datum_t magnetic_dipole_unit_vector = toml_double_at(magnetic_dipole_unit_vector_array, i);
        dipole_unit_vector[i] = magnetic_dipole_unit_vector.u.d;
    }
